...
//	private static final int DTP_OFFSET = 0x8000;
	 ...
	 * Constructor
	 */
	public EE_ElfRelocationHandler() {
		super(EE_ElfRelocationType.class);
	}

	@Override
	public boolean canRelocate(ElfHeader elf) {
		return (elf.e_machine() == ElfConstants.EM_MIPS) && ((elf.e_flags() & E_MIPS_MACH_5900) == E_MIPS_MACH_5900);
	}

	@Override
	public EE_ElfRelocationContext createRelocationContext(ElfLoadHelper loadHelper,
			Map<ElfSymbol, Address> symbolMap) {
		return new EE_ElfRelocationContext(this, loadHelper, symbolMap);
	}

	@Override
	protected RelocationResult relocate(EE_ElfRelocationContext elfRelocationContext,
		...
		if (!elfSymbol.isLocal()) {
			if (type == EE_ElfRelocationType.R_MIPS_GOT_PAGE) {
				type = EE_ElfRelocationType.R_MIPS_GOT_DISP;
				addend = 0; // addend handled by GOT_OFST
			}
			else if (type == EE_ElfRelocationType.R_MICROMIPS_GOT_PAGE) {
				type = EE_ElfRelocationType.R_MICROMIPS_GOT_DISP;
				addend = 0; // addend handled by GOT_OFST
			}
		}
		...
		switch (type) {
			...
			case R_MICROMIPS_GOT_HI16:
				...
				long appliedValue;
				if (type == EE_ElfRelocationType.R_MIPS_GOT_DISP) {
			...
			case R_MICROMIPS_GOT16:
				...
				if (elfSymbol.isLocal()) {
					...
					EE_DeferredRelocation got16reloc = new EE_DeferredRelocation(type,
			...
			case R_MICROMIPS_HI16:
				...
				EE_DeferredRelocation hi16reloc = new EE_DeferredRelocation(type, elfSymbol,
			...
			case R_MICROMIPS_HI0_LO16:
				...
				if (isGpDisp) {
					...
					if (type == EE_ElfRelocationType.R_MIPS16_LO16) {
						value -= (offset & ~0x3);
					}
					else {
						value -= offset - 4;
			...
			case R_MICROMIPS_26_S1:
				int shift = (type == EE_ElfRelocationType.R_MICROMIPS_26_S1) ? 1 : 2;
				if (elfRelocationContext.extractAddend()) {
			...
			case R_MICROMIPS_LITERAL:
				if (elfRelocationContext.extractAddend()) {
					if (type == EE_ElfRelocationType.R_MIPS_GPREL32) {
						addend = oldValue;
					}
					else {
						addend = oldValue & 0xffff;
						if (type == EE_ElfRelocationType.R_MICROMIPS_GPREL7_S2) {
							addend <<= 2;
						}
				...
				long gp0 = 0;
				if (elfSymbol.isLocal() && (type == EE_ElfRelocationType.R_MIPS_GPREL16 ||
					type == EE_ElfRelocationType.R_MIPS_GPREL32)) {
				...
				long mask = type == EE_ElfRelocationType.R_MIPS_GPREL32 ? 0xffffffffL : 0xffff;
					...
					if (block != null) {
						if (MemoryBlock.EXTERNAL_BLOCK_NAME.equals(block.getName())) {
							...
							if (success) {
								...
								if (type == EE_ElfRelocationType.R_MICROMIPS_JALR) {
	...
	private boolean isMIPS16Reloc(EE_ElfRelocationType type) {
		int typeId = type.typeId;
		return typeId >= EE_ElfRelocationType.R_MIPS16_LO.typeId &&
			typeId <= EE_ElfRelocationType.R_MIPS16_HI.typeId;
	}

	private boolean isMicroMIPSReloc(EE_ElfRelocationType type) {
		int typeId = type.typeId;
		return typeId >= EE_ElfRelocationType.R_MICROMIPS_LO.typeId &&
			typeId <= EE_ElfRelocationType.R_MICROMIPS_HI.typeId;
	}

	private boolean shuffleRequired(EE_ElfRelocationType type) {
		return isMIPS16Reloc(type) ||
			(isMicroMIPSReloc(type) && type != EE_ElfRelocationType.R_MICROMIPS_PC7_S1 &&
				type != EE_ElfRelocationType.R_MICROMIPS_PC10_S1);
	}

	private boolean isMIPS16_26_JAL_Reloc(EE_ElfRelocationType type,
			EE_ElfRelocationContext elfRelocationContext) {
		return (type == EE_ElfRelocationType.R_MIPS16_26 &&
			elfRelocationContext.getElfHeader().isRelocatable());
	}

	private int unshuffle(int value, EE_ElfRelocationType type,
			EE_ElfRelocationContext elfRelocationContext) {
		...
		else if (isMicroMIPSReloc(type) || type == EE_ElfRelocationType.R_MIPS16_26) {
	...
	private int shuffle(int value, EE_ElfRelocationType type,
			EE_ElfRelocationContext elfRelocationContext) {
		if (!shuffleRequired(type)) {
			return value;
		}
	...
	private boolean matchingHiLo16Types(EE_ElfRelocationType hi16Type,
			EE_ElfRelocationType lo16Type) {
		switch (hi16Type) {
			case R_MIPS_HI16:
			case R_MIPS_GOT16:
				return lo16Type == EE_ElfRelocationType.R_MIPS_LO16;
			case R_MIPS16_HI16:
			case R_MIPS16_GOT16:
				return lo16Type == EE_ElfRelocationType.R_MIPS16_LO16;
			case R_MICROMIPS_HI16:
			case R_MICROMIPS_GOT16:
				return lo16Type == EE_ElfRelocationType.R_MICROMIPS_LO16;
			default:
				return false;
		}
	}
	 ...
	 * @param lo16Addend
	 */
	private void processHI16Relocations(EE_ElfRelocationContext elfRelocationContext,
			EE_ElfRelocationType lo16RelocType, ElfSymbol lo16ElfSymbol, int lo16Addend) {

		Iterator<EE_DeferredRelocation> iterateHi16 = elfRelocationContext.iterateHi16();
		while (iterateHi16.hasNext()) {
			EE_DeferredRelocation hi16reloc = iterateHi16.next();
	 ...
	 */
	private void processHI16Relocation(EE_ElfRelocationContext elfRelocationContext,
			EE_DeferredRelocation hi16reloc, long lo16Addend) {
		...
		if (hi16reloc.isGpDisp) {
			...
			if (hi16reloc.relocType == EE_ElfRelocationType.R_MIPS16_HI16) {
...
// FIXME: should always use hi16reloc.addend - figure out at time of deferral
	 ...
	 * @param lo16Addend
	 */
	private void processGOT16Relocations(EE_ElfRelocationContext elfRelocationContext,
			EE_ElfRelocationType lo16RelocType, ElfSymbol lo16ElfSymbol, int lo16Addend) {

		Iterator<EE_DeferredRelocation> iterateGot16 = elfRelocationContext.iterateGot16();
		while (iterateGot16.hasNext()) {
			EE_DeferredRelocation hi16reloc = iterateGot16.next();
			if (matchingHiLo16Types(hi16reloc.relocType, lo16RelocType) &&
				hi16reloc.elfSymbol == lo16ElfSymbol) {
	 ...
	 * @return true if successful or false if unsupported
	 */
	private void processGOT16Relocation(EE_ElfRelocationContext elfRelocationContext,
			EE_DeferredRelocation got16reloc, long lo16Addend) {
	...
	private long getGpOffset(EE_ElfRelocationContext elfRelocationContext, long value) {
	...
	public void markAsUndefined(Program program, Address relocationAddress, int typeId,
			String symbolName, int symbolIndex, MessageLog log) {
		super.markAsUndefined(program, relocationAddress, typeId, symbolName, symbolIndex, log);
	}

	/**
	 * <code>EE_DeferredRelocation</code> is used to capture a relocation whose processing
	 * must be deferred.
	 */
	class EE_DeferredRelocation {

		final EE_ElfRelocationType relocType;
		...
		EE_DeferredRelocation(EE_ElfRelocationType relocType, ElfSymbol elfSymbol,
		...
		void markUnprocessed(EE_ElfRelocationContext elfRelocationContext,
