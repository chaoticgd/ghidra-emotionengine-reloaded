...
import ghidra.emotionengine.relocation.EE_ElfRelocationHandler.EE_DeferredRelocation;
...
/**
 * {@link EE_ElfRelocationContext} provides extended relocation context with the ability
 ...
 */
class EE_ElfRelocationContext extends ElfRelocationContext<EE_ElfRelocationHandler> {

	private LinkedList<EE_DeferredRelocation> hi16list = new LinkedList<>();
	private LinkedList<EE_DeferredRelocation> got16list = new LinkedList<>();
	...
	EE_ElfRelocationContext(EE_ElfRelocationHandler handler, ElfLoadHelper loadHelper,
	...
	private RelocationResult doRelocate(ElfRelocation relocation, Address relocationAddress,
		...
		EE_ElfRelocationType relocationType = handler.getRelocationType(relocType);
	...
	public void endRelocationTableProcessing() {
		...
		for (EE_DeferredRelocation reloc : hi16list) {
			reloc.markUnprocessed(this, "LO16 Relocation");
		}
		hi16list.clear();
		for (EE_DeferredRelocation reloc : got16list) {
			reloc.markUnprocessed(this, "LO16 Relocation");
		}
		got16list.clear();
	...
	boolean nextRelocationHasSameOffset(ElfRelocation relocation) {
		...
		return relocations[relocIndex].getOffset() == relocations[relocIndex + 1].getOffset() &&
			relocations[relocIndex + 1].getType() != EE_ElfRelocationType.R_MIPS_NONE.typeId;
	}
	...
	long getAdjustedGPValue() {
		...
		Symbol symbol = SymbolUtilities.getLabelOrFunctionSymbol(program,
			MIPS_ElfExtension.MIPS_GP_VALUE_SYMBOL, err -> getLog().appendMsg("EE_ELF", err));
		if (symbol == null) {
			return -1;
		}
	...
	long getGP0Value() {
		Symbol symbol = SymbolUtilities.getLabelOrFunctionSymbol(program,
			MIPS_ElfExtension.MIPS_GP0_VALUE_SYMBOL, err -> getLog().appendMsg("EE_Elf", err));
		if (symbol == null) {
			return -1;
		}
		return symbol.getAddress().getOffset();
	 ...
	 */
	Iterator<EE_DeferredRelocation> iterateHi16() {
	 ...
	 */
	void addHI16Relocation(EE_DeferredRelocation hi16reloc) {
	 ...
	 */
	Iterator<EE_DeferredRelocation> iterateGot16() {
	 ...
	 */
	void addGOT16Relocation(EE_DeferredRelocation got16reloc) {
